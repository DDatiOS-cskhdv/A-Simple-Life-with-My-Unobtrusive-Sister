...
SceneManager.initialize = function() {
	...
	Scene_Load.prototype.reloadMapIfUpdated = function() {
		...
		// 0.22A兼容
        $gameScreen._pictures.forEach(p=>{
            if (p && !p._drill_PSh_commandChangeTank){
                p._drill_PSh_commandChangeTank = [];  
            }
        });		
		
		// 避免旧存档的数据发生报错	
		let mapId = $gameMap.mapId();
		let allowRefresh = [4,11,18,21,24,44,52];	
		if ( !allowRefresh.includes(mapId) ) return;
        // 玄关场景的修复
		if (mapId === 24) {
			let eid = 3;
			$gameVariables.setValue(13, eid);
		}
		let imouto = $gameActors.actor(2);
		if ( imouto.isStateAffected(29) )  $gameVariables.setValue(13, 10);
		if ( imouto.isStateAffected(30) )  $gameVariables.setValue(13, 1);
		if ( imouto.isStateAffected(32) )  $gameVariables.setValue(13, 1);
...
Scene_Map.prototype.onMapLoaded = function() {
	...
	if ($gameTemp._drill_GSM_isInLoadScene === true) {
		...
		removedIds = [];
		var events = $gameMap && $gameMap._events;
		if (events && events.length > 0) {
			for (var i = 0; i < events.length; i++) {
				var e = events[i];
				if (!e) continue; // 兼容 undefined / null

				// 幽灵事件：Game_Event 存在，但 dataMap.events[eventId] 已被删，导致 e.event() 为 null
				var dataEv = (e.event && e.event());
				if (!dataEv) {
					// 轻量标记，避免其它逻辑误用
					e._erased = true;
					e._pageIndex = -1;
		...
		// 记录一下，后面可用于清理贴图
		$gameTemp._drill_GSM_removedEventIds = removedIds;
	}

	// > 原函数
	_drill_GSM_onMapLoaded.call(this);

	// > 可选：清理对应的 Sprite_Character（如果此时贴图已创建）
	removedIds = $gameTemp._drill_GSM_removedEventIds;
	$gameTemp._drill_GSM_removedEventIds = null;
	if (removedIds && removedIds.length > 0 && this._spriteset && this._spriteset._characterSprites) {
		var idSet = {};
		for (var k = 0; k < removedIds.length; k++) idSet[removedIds[k]] = true;

		var sprites = this._spriteset._characterSprites;
		for (var s = sprites.length - 1; s >= 0; s--) {
			var sp = sprites[s];
			var ch = sp && sp._character;
			// 只处理事件贴图（不碰玩家/队友）
			if (ch && ch.eventId && idSet[ch.eventId()]) {

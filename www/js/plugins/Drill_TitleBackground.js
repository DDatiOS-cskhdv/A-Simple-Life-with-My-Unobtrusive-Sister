...
Scene_Title.prototype.drill_TBa_create = function() {	
	...
	for (var i = 0; i < DrillUp.g_TBa_list.length; i++) {
		...
		// 适配多语言不同的UI
		let src_img = temp_sprite_data['src_img'];
		if (src_img.includes("titlesLogo") && !src_img.includes("alter")) {
		    let lang    = ConfigManager.language;
		    let allowed = window?.LANG_LOCALE?.[lang]?.titleLOGO;	
			if (!allowed) lang = 2;
			src_img  = "titlesLogo" + lang;			
			if (lang === 0 && ConfigManager.needsTC) {
				src_img += "tcn";
			}
		}
		var temp_sprite = new TilingSprite( ImageManager.loadBitmap( temp_sprite_data['src_img_file'], src_img, 0, true ) );	//TilingSprite平铺图层
		...
		if (i === 0 || i === 20) {
			...
			if ( isImouto && i === 20 )  temp_sprite.visible = false; DrillUp.global_TBa_visibleTank[0] = true;
			if ( !isImouto && i === 0 )  temp_sprite.visible = false; DrillUp.global_TBa_visibleTank[20] = true;
		}

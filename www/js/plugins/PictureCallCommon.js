...
(function() {
    ...
    Game_Map.prototype.setupPictureParallelCommonEvent = function() {
        ...
        if (event) {
			...
			*/
			let pid = $gameTemp._touchPictureId;
			$gameMap.steupCEQJ(commonId,pid);			
    ...
    Game_Screen.prototype.changePictureName = function(pictureId, newName) {		
		var picture = this.picture(pictureId);
        if (picture) {
	var oldOrigin = picture._origin;
    var oldX = picture._x;
    var oldY = picture._y;
    var oldScaleX = picture._scaleX;
    var oldScaleY = picture._scaleY;
    var oldOpacity = picture._opacity;
    var oldBlendMode = picture._blendMode;
        }
		var newPictureFile = newName;
		if (picture) {
    // 替换图片，保持原有属性
    $gameScreen.showPicture(pictureId, newPictureFile, oldOrigin, oldX, oldY, oldScaleX, oldScaleY, oldOpacity, oldBlendMode);
		}
	};
    //=============================================================================
    // SceneManager
    ...
    if (this.isPressed()) {
        ...
        // 使用欧氏距离计算
        this._pressedDistance = Math.sqrt(dx * dx + dy * dy);
    }
    this._prevX = x;
    this._prevY = y;

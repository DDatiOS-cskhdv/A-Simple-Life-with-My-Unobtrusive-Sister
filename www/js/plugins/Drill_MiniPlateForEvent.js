...
Game_Event.prototype.drill_MPFE_setupPageSettings = function() {
    ...
    if( page ){
        ...
        for(var k = 0; k < temp_list.length; k++ ){
          ...
          let m = String(row).match(/=>scriptCall\[(.*?)\]/i);
          if (m && m[1]) {
             let code = m[1].trim();
             eval(code); 
          }			
        ...
        if (row.includes("=>事件说明窗口 : 自动显示多语言文本")) {
            ...
            if (
				...
				if (window.prototypeEventTemplate[String(index)]["garble"]) {
					if (!$gameParty.leader().hasSkill(17)) {
					    descriptionArray = garble(descriptionArray);
					}
				}
            }

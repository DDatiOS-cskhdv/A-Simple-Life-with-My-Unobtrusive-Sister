...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_DSk_PluginTip_curName = "Drill_DialogSkin.js 对话框-对话框皮肤";
	DrillUp.g_DSk_PluginTip_baseList = [];
	//==============================
	// * 提示信息 - 报错 - 底层版本过低
	//==============================
...
Game_Temp.prototype.initialize = function(){
	...
	for(var i=0; i < DrillUp.g_DSk_list.length; i++ ){
		...
		if( temp_data['window_type'] == "自定义背景图片" && temp_data['window_pic_src'] != "" ){
			// 妹妹姓名框多语言适配
			/*
			if (temp_data['window_pic_src'].includes("talkback_namebox")) {
			const raw = $gameStrings?.value(120) || "";
			...
			if (isImouto) {					
			   ...
			   if (lang < 2) lang = 1;
			   if (lang > 2) lang = 2;
			   temp_data['window_pic_src'] = "talkback_namebox" + lang;
			  } else {
			   temp_data['window_pic_src'] = "talkback_namebox_alter";  
			  }
			}
			*/
...
Window_Base.prototype.drill_DSk_resetSkinData = function( data ){
	...
	if( data['window_type'] == "自定义背景图片" && data['window_pic_src'] != "" ){		
		// 妹妹姓名框多语言适配
		if (data['window_pic_src'].includes("talkback_namebox")) {
			   //data['window_pic_src'] = "talkback_namebox_null";
               let pic_x = data['window_pic_x'];
               let pic_y = data['window_pic_y'];
               let textWidth = this.drill_COWA_getTextExWidth( DrillUp.g_DNB_nameBox_suffix );
               let offsetX = 0;	
               let offsetY = 0;				   
			   switch (ConfigManager.language) {
				   case 0:
				     //DrillUp.g_DNB_nameBox_fontSize = 34;
					 offsetX = Math.round((textWidth - 64)/2);
				   break;
				   case 1:
				     offsetX = Math.round((textWidth - 64)/2);
				   break;
			   ...
			   this._drill_skin_pic_x = pic_x + offsetX;
			   this._drill_skin_pic_y = pic_y + offsetY;
		}

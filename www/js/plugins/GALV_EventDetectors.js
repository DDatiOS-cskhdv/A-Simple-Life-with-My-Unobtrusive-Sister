...
Galv.DETECT.los = function(char1,char2) {
	var a = {x:char1.centerRealX(), y:char1.centerRealY()};
	var b = {x:char2.centerRealX(), y:char2.centerRealY()};
...
Galv.DETECT.losNoFacing = Galv.DETECT.losNoFacing || function (char1, char2) {

  var a = { x: char1.centerRealX(), y: char1.centerRealY() };
  var b = { x: char2.centerRealX(), y: char2.centerRealY() };

  // —— 以下同 Galv.DETECT.los 的两段斜率扫描（水平主导/垂直主导）——
  if (Math.abs(a.x - b.x) >= Math.abs(a.y - b.y)) {
    var slope, inter;
    if (a.x === b.x) { slope = null; inter = a.x; }
    else { slope = (a.y - b.y) / (a.x - b.x); inter = a.y - slope * a.x; }

    for (var x = a.x; x <= b.x; x++) {
      var y = slope * x + inter;
      if (Galv.DETECT.isBlock(x, y)) return false;
    }
    for (var x2 = a.x; x2 >= b.x; x2--) {
      var y2 = slope * x2 + inter;
      if (Galv.DETECT.isBlock(x2, y2)) return false;
    }
  } else {
    var slope2, inter2;
    if (a.y === b.y) { slope2 = null; inter2 = a.y; }
    else { slope2 = (a.x - b.x) / (a.y - b.y); inter2 = a.x - slope2 * a.y; }

    for (var y = a.y; y <= b.y; y++) {
      var x = slope2 * y + inter2;
      if (Galv.DETECT.isBlock(x, y)) return false;
    }
    for (var y2 = a.y; y2 >= b.y; y2--) {
      var x2 = slope2 * y2 + inter2;
      if (Galv.DETECT.isBlock(x2, y2)) return false;
    }
  }
  return true;
...
Game_Character.prototype.calcDistance = function(idOrPos) {
  idOrPos = (idOrPos === undefined || idOrPos === null) ? 0 : idOrPos;

  var x1 = this.centerRealX();
  var y1 = this.centerRealY();

  var x2, y2;

  // 传入坐标对象：{x:12, y:5}
  if (typeof idOrPos === "object" && idOrPos && ("x" in idOrPos) && ("y" in idOrPos)) {
    x2 = idOrPos.x;
    y2 = idOrPos.y;
  } else {
    // 传入事件ID（>0）或玩家（<=0）
    var id = Number(idOrPos) || 0;
    var target = id > 0 ? $gameMap.event(id) : $gamePlayer;
    if (!target) return 0;

    x2 = target.centerRealX();
    y2 = target.centerRealY();
  }

  var dist = Galv.DETECT.dist(x1, y1, x2, y2) * 48;
  return dist;
};

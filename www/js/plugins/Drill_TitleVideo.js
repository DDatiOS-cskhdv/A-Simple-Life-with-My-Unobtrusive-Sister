...
Drill_TVi_VideoSprite.prototype.drill_TVi_videoEnded = function() {
    ...
    // 可选：强制将 currentTime 设置为结尾，确保停在最后一帧
    this._drill_src.currentTime = this._drill_src.duration || this._drill_loopEnd;
...
Drill_TVi_VideoSprite.prototype.drill_TVi_destroy = function() {
    // 1. 解绑所有事件监听
    if (this._drill_src && this._drill_videoTimeupdateHandler) {
        this._drill_src.removeEventListener('timeupdate', this._drill_videoTimeupdateHandler);
        this._drill_videoTimeupdateHandler = null;
    }
    if (this._drill_src && this._drill_videoErrorHandler) {
        this._drill_src.removeEventListener('error', this._drill_videoErrorHandler);
        this._drill_videoErrorHandler = null;
    }
    // 2. 停止并彻底销毁 video 对象
    if (this._drill_src) {
        this._drill_src.pause();
        this._drill_src.muted = true;
        if (this._drill_src.parentNode) this._drill_src.parentNode.removeChild(this._drill_src);
        this._drill_src.remove();
        this._drill_src.src = "";
        this._drill_src.load();
        this._drill_src = null;
    }
    // 3. 销毁PIXI贴图
    if (this.bitmap && this.bitmap.baseTexture) {
        this.bitmap.baseTexture.destroy();
        this.bitmap = null;
    }
    // 4. 关键：从父容器移除自己（防止残影）
    if (this.parent) {
...
// 防范点击新游戏时存在帧画面停留于画面的问题
var _drill_TVi_commandNewGame = Scene_Title.prototype.commandNewGame;
Scene_Title.prototype.commandNewGame = function() {
    // 销毁所有视频精灵
    if ($gameTemp._drill_TVi_sprites && $gameTemp._drill_TVi_sprites.length > 0) {
        for (var i = 0; i < $gameTemp._drill_TVi_sprites.length; i++) {
            var sprite = $gameTemp._drill_TVi_sprites[i];
            if (sprite) sprite.drill_TVi_destroy();
        }
        $gameTemp._drill_TVi_sprites = [];
    }
    // 原有流程
    _drill_TVi_commandNewGame.call(this);
};
...
/*:
 // Drill_TitleVideo 修复离开标题时报 WebGL no video，并统一隔帧更新
 */

(function () {
  if (!window.Drill_TVi_VideoSprite) return;

  // 统一隔 N 帧更新一次（2=隔1帧，3=隔2帧）
  var UPDATE_EVERY_N_FRAMES = 40;

  var P = Drill_TVi_VideoSprite.prototype;

  // --- 1) 覆盖 update：加上“就绪判定 + 隔帧” ---
  P.update = function () {
    if (!(SceneManager._scene instanceof Scene_Title)) return;
    Sprite.prototype.update.call(this);

    // 被销毁/正在切场景时，不要再上传纹理
    if (this._drill__destroyed) return;
    if (SceneManager.isSceneChanging && SceneManager.isSceneChanging()) return;

    if (!this._drill_texture_loaded) return;
    if (!this._drill_texture) return;

    // 统一降频
    if ((Graphics.frameCount % UPDATE_EVERY_N_FRAMES) !== 0) return;

    // video 必须是真就绪（避免 no video / 0x0 帧）
    var v = this._drill_src;
    if (!v || v.readyState < 2 || v.videoWidth <= 0 || v.videoHeight <= 0) return;

    try { this._drill_texture.update(); } catch (e) {}
  };

  // --- 2) 覆盖 destroy：真正销毁 _drill_texture，并断开 _drill_video.texture ---
  var _oldDestroy = P.drill_TVi_destroy;
  P.drill_TVi_destroy = function () {
    this._drill__destroyed = true;
    this._drill_texture_loaded = false;

    // 先把子 sprite 解绑纹理，避免底层还引用着 video texture
    try {
      if (this._drill_video) this._drill_video.texture = PIXI.Texture.EMPTY;
    } catch (e) {}
    ...
    // 真正销毁“视频纹理”（原插件 destroy 用 bitmap，很可能没打到点）
    var tex = this._drill_texture;
    if (tex) {
      // 顺便从 PIXI cache 里删掉，避免残留
      var key = this._drill_data && this._drill_data.path;
      try {
        if (key && PIXI.utils && PIXI.utils.TextureCache) delete PIXI.utils.TextureCache[key];
        if (key && PIXI.utils && PIXI.utils.BaseTextureCache) delete PIXI.utils.BaseTextureCache[key];
      } catch (e) {}

      try { tex.destroy(true); } catch (e) {
        try { tex.baseTexture && tex.baseTexture.destroy(); } catch (e2) {}
      }
    }
    this._drill_texture = null;

    // 销毁内部视频 sprite
    if (this._drill_video) {
      try { this._drill_video.destroy({ children: true }); } catch (e) {}
      this._drill_video = null;
    }

    // 从父容器移除
    if (this.parent) this.parent.removeChild(this);

    // 如果你还想保留原 destroy 里其它逻辑，可以最后再调用一次
    // （但注意：原逻辑可能会再次操作已 null 的对象，所以包 try）
    try { if (_oldDestroy) _oldDestroy.call(this); } catch (e) {}
  };

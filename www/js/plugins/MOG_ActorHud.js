...
Actor_Hud.prototype.load_img = function () {
	...
	this._state_img = ImageManager.loadSystem("IconSet_large");
...
Actor_Hud.prototype.create_layout = function () {
	...
	this._layout = new Sprite_Button();
	this._layout.bitmap = this._layout_img;
	//this._layout.setClickHandler(this.onLayoutPressed.bind(this));
	this.addChild(this._layout);
};

Actor_Hud.prototype.onLayoutPressed = function () {
...
Actor_Hud.prototype.create_states = function () {
	...
	if (!this._battler) { return };
	this._states_data = [0, 0, 0];
	this._state_icons = []; // 创建一个数组来保存状态图标
	this._battler.states().forEach(function (state) {
		const iconIndex = state?.iconIndex || 0;
		if (iconIndex <= 0) return;
        this._state_icon = new Sprite(new Bitmap(64, 64));
		this._state_icon.scale.x = 0.5;
		this._state_icon.scale.y = 0.5;
		var sx = iconIndex % 16 * 64;
		var sy = Math.floor(iconIndex / 16) * 64;
		this._state_icon.bitmap.blt(this._state_img, sx, sy, 64, 64, 0, 0);

		//this._state_icon.setFrame(sx, sy, 64, 64);
		this._state_icon.x = this._pos_x + Moghunter.ahud_states_pos_x;//this._pos_x + Moghunter.ahud_states_pos_x + (this._state_icon.width * 0.5 * this._state_icons.length);
		this._state_icon.y = this._pos_y + Moghunter.ahud_states_pos_y;//this._pos_y + Moghunter.ahud_states_pos_y;
		this._state_icons.push(this._state_icon); // 将新创建的状态图标添加到数组中
...
Actor_Hud.prototype.refresh_states = function () {
	...
	// 清除旧的状态图标
	this._state_icons.forEach(function (icon) {
	...
	this._battler.states().forEach(function (state) {
		...
		var state_icon = new Sprite(new Bitmap(64, 64));
		state_icon.scale.x = 0.5;
		state_icon.scale.y = 0.5;
		var sx = iconIndex % 16 * 64;
		var sy = Math.floor(iconIndex / 16) * 64;
		state_icon.bitmap.blt(this._state_img, sx, sy, 64, 64, 0, 0);
		
		//添加状态回合数
		var stateCounter = $gameSystem.getTimeEventRemainingMinutes(`state${state.id}`);
        if (stateCounter > 0 && stateCounter < 4) {
		const nBit = new Bitmap(32, 32);
        nBit._context.shadowColor = 'rgba(0, 0, 0, 1)';  
        nBit._context.shadowBlur = 10;
        nBit._context.shadowOffsetX = 2;
        nBit._context.shadowOffsetY = 2;		
		nBit.fontSize = 30;
		nBit.fontFace = "MPLUS2ExtraBold";
		nBit.textColor = "#b7b7b7";
		nBit.drawText(stateCounter, 0, 0, 32, 32, 'center');
		state_icon.bitmap.blt(nBit, 0, 0, 32, 32, 16, 16);
       }		
		//state_icon.setFrame(sx, sy, 64, 64);
		state_icon.x = this._pos_x + Moghunter.ahud_states_pos_x + (state_icon.width * 0.5 * this._state_icons.length);
		state_icon.y = this._pos_y + Moghunter.ahud_states_pos_y;
...
Actor_Hud.prototype.update_states = function () {
	if (this._battler.allIcons().length == 0 && this._state_icons.length == 0) { return };
...
Actor_Hud.prototype.need_refresh_states = function () {
	...
	if (this._states_data[2] > 40) { return true };
	return false;
};
...
Game_Interpreter.prototype.drill_GFTH_command126_withVariable = function(itemId, valueChangeType, value) {
    var item_id = itemId; 
    var realValue = this.operateValue(valueChangeType, 0, value);
    $gameParty.gainItem($dataItems[item_id], realValue); 

    // > 文本生成
    var context = this.drill_GFTH_getText_item(item_id, realValue);
    if (context != "") {
        $gameTemp.drill_GFTH_pushNewText(context);
    }
...
Game_Interpreter.prototype.drill_GFTH_command127_withVariable = function(itemId, valueChangeType, value) {
    var item_id = itemId; 
    var realValue = this.operateValue(valueChangeType, 0, value);
    $gameParty.gainItem($dataWeapons[item_id], realValue); 

    // > 文本生成
    var context = this.drill_GFTH_getText_weapon(item_id, realValue);
    if (context != "") {
        $gameTemp.drill_GFTH_pushNewText(context);
    }
...
Game_Interpreter.prototype.drill_GFTH_command128_withVariable = function(itemId, valueChangeType, value) {
    var item_id = itemId; 
    var realValue = this.operateValue(valueChangeType, 0, value);
    $gameParty.gainItem($dataArmors[item_id], realValue); 

    // > 文本生成
    var context = this.drill_GFTH_getText_armor(item_id, realValue);
    if (context != "") {
        $gameTemp.drill_GFTH_pushNewText(context);
    }
	...
	if( count > 0 && data['item_gain_enabled'] == true ){
		...
		if( data['item_text_type'] == "图标+文本" ){
			...
			if (ConfigManager.language > 1) {
				context += "\\fs[32]\\i[" + item.iconIndex + "]\\fs[18]";
			} else {
				context += "\\fs[32]\\i[" + item.iconIndex + "]\\fs[22]";
			}
			context += name_str;
			//context += " x ";
			//context += "\\c[" + data['gain_color'] + "]" + count;
...
Game_Interpreter.prototype.drill_GFTH_getText_weapon = function( weapon_id, count ){
	...
	if( count > 0 && data['weapon_gain_enabled'] == true ){
		...
		if( data['weapon_text_type'] == "图标+文本" ){
			if (ConfigManager.language > 1) {
				context += "\\fs[28]\\i[" + weapon.iconIndex + "]\\fs[18]";
			} else {
				context += "\\fs[28]\\i[" + weapon.iconIndex + "]\\fr";
			}
			context += name_str;
			//context += " x ";
			//context += "\\c[" + data['gain_color'] + "]" + count;
	...
	if( count < 0 && data['weapon_lost_enabled'] == true ){
		...
		if( data['weapon_text_type'] == "图标+文本" ){
			if (ConfigManager.language > 1) {
				context += "\\fs[32]\\i[" + weapon.iconIndex + "]\\fs[18]";
			} else {
				context += "\\fs[32]\\i[" + weapon.iconIndex + "]\\fs[22]";
			}
			context += name_str;
			context += " x ";
			context += "\\c[" + data['lost_color'] + "]" + count;
...
Game_Interpreter.prototype.drill_GFTH_getText_armor = function( armor_id, count ){
	...
	if( count > 0 && data['armor_gain_enabled'] == true ){
		...
		if( data['armor_text_type'] == "图标+文本" ){
			if (ConfigManager.language > 1) {
				context += "\\fs[32]\\i[" + armor.iconIndex + "]\\fs[18]";
			} else {
				context += "\\fs[32]\\i[" + armor.iconIndex + "]\\fs[22]";
			}
			context += name_str;
			//context += " x ";
			//context += "\\c[" + data['gain_color'] + "]" + count;

...
Scene_Title.prototype.createCommandWindow = function() {
	...
	this._commandWindow.setHandler('Links', this.externalLinks.bind(this));
};
...
Scene_Title.prototype.externalLinks = function() {	
    chahuiUtil.openOfficialWebsite?.();		
...
Window_TitleCommand.prototype.makeCommandList = function() {
	...
	this.addCommand('SUPPORT', 'Links'); // 追加外部链接
...
/*
//* ------------------------------------------------------------------
//* TitleCommand 文本 Glow + 文本拦截钩子
//* ------------------------------------------------------------------ 
(function(){
  'use strict';

  // 命名空间（暴露给其他脚本/多语言系统）
  window.Drill_TitleScene = window.Drill_TitleScene || {};
  // 发光配置（可在运行时修改）
  Drill_TitleScene._glowEnabled = true;
  Drill_TitleScene._glowColor   = '#000000';
  Drill_TitleScene._glowBlur    = 16;

  // 文本拦截钩子（外部可注入：fn({ text, symbol, index, window }) => string|undefined）
  Drill_TitleScene._labelHook = null;
  Drill_TitleScene.setTitleCommandTextHook = function(fn){
    this._labelHook = (typeof fn === 'function') ? fn : null;
  };
  Drill_TitleScene.setTitleGlow = function(opt){
    if (!opt) return;
    if (typeof opt.enabled !== 'undefined') this._glowEnabled = !!opt.enabled;
    if (typeof opt.color   === 'string')    this._glowColor   = opt.color;
    if (typeof opt.blur    === 'number')    this._glowBlur    = Math.max(0, opt.blur|0);
  };

  // 覆盖绘制：只改绘制流程，不动命令列表/对齐/光标等
  const _orig_drawItem = Window_TitleCommand.prototype.drawItem;
  Window_TitleCommand.prototype.drawItem = function(index){
	...
	console.log(text);
    console.log(index);
	//if (index == 1) text = "读取游戏";
    // 文本拦截（让外部多语言系统有机会替换显示文本）
    if (Drill_TitleScene._labelHook){
      try {
        const t2 = Drill_TitleScene._labelHook({ text, symbol, index, window: this });
        if (typeof t2 === 'string') text = t2;
      } catch(e){ console.error('[TitleScene labelHook]', e); }
    }

    // Glow 绘制
    const c   = this.contents;
    const ctx = c._context;
    const ow0 = c.outlineWidth;
    const oc0 = c.outlineColor;

    // 关描边，用阴影实现外发光；保留你原来的对齐/行高
    c.outlineWidth = 0;
    if (Drill_TitleScene._glowEnabled) {
      ctx.save();
      ctx.shadowColor   = Drill_TitleScene._glowColor;
      ctx.shadowBlur    = Drill_TitleScene._glowBlur;
      ctx.shadowOffsetX = 0;
      ctx.shadowOffsetY = 0;
    }

    // 调用窗口的标准绘制（会走 this.drawText -> contents.drawText）
    this.resetTextColor();
    this.drawText(text, rect.x, rect.y, rect.width, this.itemTextAlign());

    if (Drill_TitleScene._glowEnabled) ctx.restore();
    c.outlineWidth = ow0; 
    c.outlineColor = oc0;
  };

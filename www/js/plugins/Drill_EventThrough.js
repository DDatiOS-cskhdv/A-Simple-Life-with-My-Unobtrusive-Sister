...
Game_Event.prototype.setupPage = function() {
    ...
    this.drill_ETh_event_readPage();
};
//==============================
// * 事件注释 - 读取 页
//==============================
Game_Event.prototype.drill_ETh_event_readPage = function() {	
	
	// > 第一次出生，强制读取第一页注释（防止离开地图后，回来，开关失效）
	if( !this._erased && this.event() && this.event().pages[0] && this._drill_ETh_isFirstBirth == true ){ 
		this.drill_ETh_event_readList( this.event().pages[0].list );
		this._drill_ETh_isFirstBirth = undefined;		//『节约临时参数存储空间』（放后面，注释通过这个识别"跨事件页/不跨事件页"。"跨事件页"的注释必须放在第一页才能生效。）
	}
	
	// > 读取当前页注释
	if( !this._erased && this.page() ){ 
		this.drill_ETh_event_readList( this.list() );
	}
}
//==============================
// * 事件注释 - 读取 注释
//==============================
Game_Event.prototype.drill_ETh_event_readList = function( pageOfList ){
...
SceneManager.initialize = function() {
  ...
  // 移除指定类型标签
Game_CharacterBase.prototype.drill_ETh_removeTagsByPattern = function(pattern, opt){
    if (!this.drill_ETh_getTagList || !this.drill_ETh_removeTag) return 0;

    const options = Object.assign({ caseSensitive:false }, opt||{});
    const listSnapshot = this.drill_ETh_getTagList();
    if (!Array.isArray(listSnapshot) || listSnapshot.length === 0) return 0;

    // 构造测试器
    let tester;
    if (pattern instanceof RegExp) {
      tester = (s)=> pattern.test(String(s));
    } else {
      const needle = String(pattern != null ? pattern : "servant");
      if (options.caseSensitive) {
        tester = (s)=> String(s).includes(needle);
      } else {
        const low = needle.toLowerCase();
        tester = (s)=> String(s).toLowerCase().includes(low);
      }
    }
    ...
    for (let i=0;i<listSnapshot.length;i++){
      const tag = listSnapshot[i];
      if (tester(tag)) {
        try { this.drill_ETh_removeTag(tag); removed++; } catch(e){}
      }
    }
    return removed;
  };

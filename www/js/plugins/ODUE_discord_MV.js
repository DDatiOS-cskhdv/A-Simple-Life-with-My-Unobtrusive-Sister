...
Utils.isWindows = function () {
  if (!Utils.isNwjs() || Utils.isMobileDevice()) return false;

  // --- Node/NW.js 的一手信息（最可靠） ---
  try {
    if (typeof process !== 'undefined' && process.platform) {
      return process.platform === 'win32';    // 仅 Windows
    }
    if (typeof require === 'function') {
      const os = require('os');
      return os.platform && os.platform() === 'win32';
    }
  } catch (_) { /* ignore */ }
  ...
  const isMac   = /macintosh|mac os x/.test(ua) || /mac/.test(pf);
  const isLinux = /linux|x11/.test(ua)        || /linux|x11/.test(pf);
  const isCrOS  = /cros/.test(ua);            // ChromeOS
  const isAndroid = /android/.test(ua);

  if (isMac || isLinux || isCrOS || isAndroid) return false;

  // 常见的 Windows 迹象（兜底）
  const isWinUA = /windows nt|win64|wow64|win32/.test(ua) || /win/.test(pf);
  return !!isWinUA;
};
...
if (Utils.isMobileDevice()) {
	Imported.ODUE_discord_MV = false;
} else {
    Imported.ODUE_discord_MV = true;
}

...
DataManager.processItemCoreNotetags = function(group) {
  ...
  var note2 = /<(?:BE INDEPENDENT ITEM|be independent item)>/i;
...
DataManager.isIndependent = function(item) {
    ...
    if (item.beIndependent) return true;
...
DataManager.addNewIndependentItem = function(baseItem, newItem) {
	...
	$gameVariables.setValue(305, newItem.id);
...
Window_ItemList.prototype.updateHelp = function() {
	// 先跑 Info Eval（只在选中变化时跑一次）
	if (this._lastInfoEvalItem !== this._index) {
	  this._lastInfoEvalItem = this._index;
	  let item = this.item();
	  runInfoEvalWithItemThis(item); // 这里保持 this=道具本身
	}	
...
function runInfoEvalWithItemThis(item) {
	if (!item) return;

	// 保持与 YEP 原逻辑一致：从 base item 继承 infoEval :contentReference[oaicite:2]{index=2}
	if (item.infoEval === undefined) {
	  item.infoEval = DataManager.getBaseItem(item).infoEval;
	}
	if (!item.infoEval) return;

	// 保持与 YEP 原 preInfoEval 提供的局部变量一致 :contentReference[oaicite:3]{index=3}
	var weapon = item;
	var armor  = item;
	var s = $gameSwitches._data;
	var v = $gameVariables._data;
	...
	try {
	  // 关键：让 this = item，同时也提供 item/weapon/armor/s/v 这些局部变量
	  var fn = new Function('item', 'weapon', 'armor', 's', 'v', code);
	  fn.call(item, item, weapon, armor, s, v);
	} catch (e) {
	  // 用 YEP 的报错显示方式更友好
	  if (window.Yanfly && Yanfly.Util && Yanfly.Util.displayError) {
		Yanfly.Util.displayError(e, code, 'ITEMLIST ON-SELECT INFO EVAL ERROR');
	  } else {
...
DataManager.cleanupIndependentDatabaseItems = function() {
    // 对武器
    var baseWeaponsLength = this._baseWeaponsLength; // 基底武器数量
    // 清空独立武器数组
    this._independentWeapons = [];
    // 更新 $dataWeapons，只保留基底武器
    $dataWeapons = $dataWeapons.slice(0, baseWeaponsLength);

    // 对护甲
    var baseArmorsLength = this._baseArmorsLength; // 基底护甲数量
    this._independentArmors = [];
    $dataArmors = $dataArmors.slice(0, baseArmorsLength);
};

Game_Party.prototype.cleanupIndependentPartyItems = function() {
    // 先对所有队员执行解除装备操作
    ...
    // 然后清空存储独立装备的记录
    var baseWeaponsLength = DataManager._baseWeaponsLength;
    for (var key in this._weapons) {
        if (parseInt(key) >= baseWeaponsLength) {
            delete this._weapons[key];
        }
    }
    
    var baseArmorsLength = DataManager._baseArmorsLength;
    for (var key in this._armors) {
        if (parseInt(key) >= baseArmorsLength) {
            delete this._armors[key];
        }
    }
};
...
DataManager.isDatabaseLoaded = function() {
  ...
  if (weaponTemplate === undefined) {
    weaponTemplate = clone($dataWeapons[290]);
    armorTemplate  = clone($dataArmors[290]);
  }
  return true;
};
...
function makeArmorStub(id) {
  ...
  if (!a) {
    a = {
      ...
      iconIndex:0, name:'error item', note:'', params:[0,0,0,0,0,0,0,0],
      price:0, meta:{}
    };
  }
  a.id = id;

...
(function (DotMoveSystem) {
    ...
    class EventDotMoveTempData extends CharacterDotMoveTempData {
        ...
        initialize(character) {
            ...
            this._widthArea = values.widthArea != null ? parseFloat(values.widthArea) : 0.2;
            this._heightArea = values.heightArea != null ? parseFloat(values.heightArea) : 0.3;
    ...
    Game_Map.prototype.update = function (sceneActive) {
        for (const character of this.allCharacters()) {
			if (!character || typeof character.prepareUpdate !== 'function') continue; // 防护
    ...
    Game_CharacterBase.prototype.initMembers = function () {
        ...
        this._moved = false;
        this._moving = false;
		this._dotMoving = false;
		this._dotMovingCounter = 0;
    ...
    if (this._dotMovingCounter > 0) {
        this._dotMovingCounter -= 1; 
        this._dotMoving = true;
    } else {
        this._dotMoving = false; 
    }
        if (!this.isMoving())
    ...
    Game_CharacterBase.prototype.isDotMoving = function () {
        return this._dotMoving;
    };
    ...
    Game_Character.prototype.dotMoveToTarget = function (eventId) {
        const deg = this.calcDeg($gameMap.event(eventId));
        this.dotMoveByDeg(deg);
    };
    ...
    Game_Event.prototype.setupCollideTriggerEventIds = function () {
    const tempData = this.dotMoveTempData();
    let collideTriggerEventIds = $gameNumberArray.value(33);
    collideTriggerEventIds = [];
    for (const result of this.checkHitCharacters(Game_Event)) {
        const event = result.targetObject;
        const eventId = event.eventId();
        if (result.collisionLengthX() >= event.widthArea() && result.collisionLengthY() >= event.heightArea()) {
            collideTriggerEventIds.push(eventId);
        }
    }
    $gameNumberArray.setValue(33, collideTriggerEventIds);
    };

...
(function(){
  // 通用：只保留 equips()[0] 参与计算
  Game_Actor.prototype.paramKeepMainWeapon = function(paramId){
    // 备份
    const _equips = this.equips;
    const slotsLen = this.equipSlots().length;
    const cur = this.equips();                 // 当前完整装备（含 null）
    const main = cur[0] || null;               // 只保留第 0 槽（主武器）
    const ghost = Array(slotsLen).fill(null);  // 其它槽位置为 null
    ghost[0] = main;

    // 清缓存并临时替换 equips()
    this._baseParamCache = undefined;
    this.equips = function(){ return ghost; };

    try {
      // 直接用 YEP 覆写过的 param()，保证与你项目公式一致
      return Game_BattlerBase.prototype.param.call(this, paramId);
    } finally {
      // 还原
      this.equips = _equips;
      this._baseParamCache = undefined;
  ...
  // 便捷：MAT（魔法攻击力，MV 的 paramId=4）
  Game_Actor.prototype.matKeepMainWeapon = function(){
    return this.paramKeepMainWeapon(4);
  };
})();


/* Average Cap for YEP_BaseParamControl: 多来源 <param Max:x> 取平均值（向下取整） */
(function () {
  "use strict";

  function avgCap(list) {
    const arr = [];
    for (const v of list) if (v != null && v > 0) arr.push(Number(v));
    if (arr.length === 0) return 0;                    // 没自定义上限 → 走默认上限
  ...
  Game_Battler.prototype.customParamMax = function (paramId) {
    ...
    // 来自脚本 API（YEP 提供的 setCustomXxxMax）
    const fromScript = Game_BattlerBase.prototype.customParamMax.call(this, paramId);
    if (fromScript > 0) caps.push(fromScript);

    // 来自状态注释
    const sts = this.states();
    for (let i = 0; i < sts.length; i++) {
      const st = sts[i];
      if (st && st.maxParams && st.maxParams[paramId] > 0) {
        caps.push(st.maxParams[paramId]);
      }
    }
    return avgCap(caps);
  };

  // 角色：合并 角色/职业/装备 +（上面 battler 的状态/脚本）
  Game_Actor.prototype.customParamMax = function (paramId) {
    const caps = [];

    // 继承状态/脚本的聚合
    const fromBattler = Game_Battler.prototype.customParamMax.call(this, paramId);
    if (fromBattler > 0) caps.push(fromBattler);

    // 角色 & 职业
    const a = this.actor();        if (a && a.maxParams && a.maxParams[paramId] > 0) caps.push(a.maxParams[paramId]);
    const c = this.currentClass(); if (c && c.maxParams && c.maxParams[paramId] > 0) caps.push(c.maxParams[paramId]);

    // 装备（会按穿戴个数重复计入）
    const eqs = this.equips();
    for (let i = 0; i < eqs.length; i++) {
      const eq = eqs[i];
      if (eq && eq.maxParams && eq.maxParams[paramId] > 0) {
        caps.push(eq.maxParams[paramId]);
      }
    }
  ...
  Game_Enemy.prototype.customParamMax = function (paramId) {
    ...
    const e = this.enemy();
    if (e && e.maxParams && e.maxParams[paramId] > 0) caps.push(e.maxParams[paramId]);

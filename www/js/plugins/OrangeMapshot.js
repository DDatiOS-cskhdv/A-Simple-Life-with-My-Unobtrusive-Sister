...
window.OrangeMapshot = OrangeMapshot; // 将 OrangeMapshot 暴露到全局作用域
(function($) {
  "use strict";
...
/* OrangeMapshot 事件绘制修正：使用 _realX/_realY 绝对坐标，避免错位 */
(function() {
  if (!Tilemap.prototype._paintCharacters) return;

  const drawOne = function(bitmap, spr, pri){
    if (!(spr instanceof Sprite_Character)) return;
    const ch = spr._character;
    if (!ch) return;
    // 跳过玩家/跟随者/载具（与原逻辑一致）
    if (ch instanceof Game_Player || ch instanceof Game_Follower || ch instanceof Game_Vehicle) return;
    spr.update();
    if (spr._characterName === '' && spr._tileId === 0) return;
    if (pri !== undefined && ch._priorityType !== pri) return;

    const tw = $gameMap.tileWidth();
    const th = $gameMap.tileHeight();
    const fr = spr._frame;                     // 当前帧裁切矩形
    const ax = (spr.anchor && spr.anchor.x != null) ? spr.anchor.x : 0.5;
    const ay = (spr.anchor && spr.anchor.y != null) ? spr.anchor.y : 1.0;
    const shiftY = (typeof ch.shiftY === 'function') ? ch.shiftY() : 0;

    // 直接用真实地图坐标：左上角 = 中心(或底部) - 锚点尺寸
    const worldCenterX = ch._realX * tw + tw * 0.5;
    const worldBottomY = ch._realY * th + th - shiftY;

    const drawX = Math.round(worldCenterX - fr.width * ax);
    const drawY = Math.round(worldBottomY - fr.height * ay);

    bitmap.blt(spr.bitmap, fr.x, fr.y, fr.width, fr.height, drawX, drawY);
  };

  // 覆盖原来的事件绘制函数（保留原有的分层接口）
  const _paintCharacters = Tilemap.prototype._paintCharacters;
  Tilemap.prototype._paintCharacters = function(bitmap, priority) {
    // 不再用 child.x/child.y + displayX/Y 倒推，逐个用 _realX/_realY 绘制
    for (let i = 0; i < this.children.length; i++) {
      drawOne(bitmap, this.children[i], priority);
    }
  };
})();

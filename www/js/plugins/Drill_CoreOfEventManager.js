		...
		if( e_ids != null && args.length == 4 ){
			...
			if( type == "彻底删除" ){
				for( var k=0; k < e_ids.length; k++ ){
					...
					$gameMap.clearBalloonForEvent(e_id);
					$gameSelfSwitches.drill_COEM_deleteEventKeys( this._mapId, e_id );
					$gameSelfVariables.clearVariablesForEvent(this._mapId, e_id);
...
// * 删除流程 - 删除未播放完毕的气泡动画
//==============================
Game_Map.prototype.clearBalloonForEvent = function(eventId) {
  ...
  for (const sp of spriteset._characterSprites) {
    if (sp && sp._character === ev) {
      if (sp._balloonSprite) {
        if (sp.endBalloon) sp.endBalloon();
        else { sp.removeChild(sp._balloonSprite); sp._balloonSprite = null; }
      }
      break;
    }
  }
...
//适配独立变量插件的新方法，用于定期删除无用数据
Game_Map.prototype.cleanUpSelfVariables = function() {
        var selfVariables = window.Game_SelfVariables; // 获取 Game_SelfVariables 实例
        var mapId = this._mapId;
        var validEventIds = this.events().map(function(event) {
            return event.eventId();
        });

        for (var key in selfVariables._data) {
            if (selfVariables._data.hasOwnProperty(key)) {
                ...
                if (storedMapId === mapId && !validEventIds.includes(storedEventId)) {
                    // 如果事件不在当前地图的事件列表中，删除该独立变量
                    delete selfVariables._data[key];
                }
            }
        }
